...
- Lookup field display via `displayLookup()` / `displayMultiLookup()` in `src/lib/formatters.ts`
...
`lookup/select` and `lookup/radio` fields return `{key, label}` objects from the API. `multiplelookup/checkbox` returns arrays of `{key, label}`.

```typescript
import { displayLookup, displayMultiLookup } from '@/lib/formatters';

// Display in JSX:
<span>{displayLookup(record.fields.kategorie)}</span>       // → "Gemüse"
<span>{displayMultiLookup(record.fields.allergene)}</span>   // → "Gluten, Laktose"
```

**NEVER render lookup fields directly** — always use `displayLookup()` / `displayMultiLookup()`.
