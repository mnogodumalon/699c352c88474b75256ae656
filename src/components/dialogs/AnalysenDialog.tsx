...
export function AnalysenDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = false }: AnalysenDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      await onSubmit(fields as Analysen['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "analyse_titel": string | null, // Titel der Analyse\n  "produktbild": string | null, // Produktbild hochladen\n  "analysedatum": string | null, // YYYY-MM-DDTHH:MM // Analysedatum und -zeit\n  "gesamtbewertung": string | null, // Gesamtbewertung\n  "allgemeine_notizen": string | null, // Allgemeine Notizen\n}`;
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{defaultValues ? 'Analysen bearbeiten' : 'Analysen hinzufügen'}</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              id="produktbild"
              value={fields.produktbild ?? ''}
              onChange={e => setFields(f => ({ ...f, produktbild: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={v => setFields(f => ({ ...f, gesamtbewertung: v === 'none' ? undefined : v as 'sehr_empfehlenswert' | 'empfehlenswert' | 'akzeptabel' | 'nicht_empfehlenswert' | 'bedenklich' }))}
            >
