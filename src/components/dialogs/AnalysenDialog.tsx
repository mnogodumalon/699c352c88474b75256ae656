...
interface AnalysenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Analysen['fields']) => Promise<void>;
  defaultValues?: Analysen['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function AnalysenDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = false, enablePhotoLocation = true }: AnalysenDialogProps) {
  const [fields, setFields] = useState<Partial<Analysen['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'analysen');
      await onSubmit(clean as Analysen['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "analyse_titel": string | null, // Titel der Analyse\n  "analysedatum": string | null, // YYYY-MM-DDTHH:MM\n  "gesamtbewertung": LookupValue | null, // Gesamtbewertung (select one key: "sehr_empfehlenswert" | "empfehlenswert" | "akzeptabel" | "nicht_empfehlenswert" | "bedenklich") mapping: sehr_empfehlenswert=Sehr empfehlenswert, empfehlenswert=Empfehlenswert, akzeptabel=Akzeptabel, nicht_empfehlenswert=Nicht empfehlenswert, bedenklich=Bedenklich\n  "allgemeine_notizen": string | null, // Allgemeine Notizen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Analysen['fields']>;
      });
      ...
      if (file.type.startsWith('image/') || file.type === 'application/pdf') {
        try {
          ...
          setFields(prev => ({ ...prev, produktbild: fileUrl }));
        } catch (uploadErr) {
  ...
  const DIALOG_INTENT = defaultValues ? 'Analysen bearbeiten' : 'Analysen hinzufügen';
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="analyse_titel">Titel der Analyse</Label>
            <Input
              id="analyse_titel"
              value={fields.analyse_titel ?? ''}
              onChange={e => setFields(f => ({ ...f, analyse_titel: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="produktbild">Produktbild hochladen</Label>
            {fields.produktbild ? (
              <div className="flex items-center gap-3 rounded-lg border p-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm truncate text-foreground">{fields.produktbild.split("/").pop()}</p>
                  <div className="flex gap-2 mt-1">
                      ...
                      <input
                        ...
                        onChange={async (e) => {
                          ...
                          try {
                            ...
                            setFields(f => ({ ...f, produktbild: fileUrl }));
                    ...
                    <button
                      ...
                      onClick={() => setFields(f => ({ ...f, produktbild: undefined }))}
                    >
                      Entfernen
                    </button>
                  </div>
          ...
          <div className="space-y-2">
            <Label htmlFor="analysedatum">Analysedatum und -zeit</Label>
            <Input
              id="analysedatum"
              type="datetime-local"
              step="60"
          ...
          <div className="space-y-2">
            <Label htmlFor="gesamtbewertung">Gesamtbewertung</Label>
            <Select
