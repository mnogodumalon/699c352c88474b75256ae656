...
export function SchnellanalyseDialog({ open, onClose, onSubmit, defaultValues, zutatendatenbankList, enablePhotoScan = false, enablePhotoLocation = true }: SchnellanalyseDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'schnellanalyse');
      await onSubmit(clean as Schnellanalyse['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="erkannte_zutaten" entity="Zutatendatenbank">\n${JSON.stringify(zutatendatenbankList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      if (file.type.startsWith('image/') || file.type === 'application/pdf') {
        try {
          ...
          setFields(prev => ({ ...prev, schnell_bild: fileUrl }));
        } catch (uploadErr) {
  ...
  const DIALOG_INTENT = defaultValues ? 'Schnellanalyse bearbeiten' : 'Schnellanalyse hinzufügen';
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {fields.schnell_bild ? (
              <div className="flex items-center gap-3 rounded-lg border p-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm truncate text-foreground">{fields.schnell_bild.split("/").pop()}</p>
                  <div className="flex gap-2 mt-1">
                      ...
                      <input
                        ...
                        onChange={async (e) => {
                          ...
                          try {
                            ...
                            setFields(f => ({ ...f, schnell_bild: fileUrl }));
                    ...
                    <button
                      ...
                      onClick={() => setFields(f => ({ ...f, schnell_bild: undefined }))}
                    >
                      Entfernen
                    </button>
                  </div>
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={v => setFields(f => ({ ...f, schnell_gesamtbewertung: v === 'none' ? undefined : v as any }))}
            >
