...
export function AnalyseergebnisseDialog({ open, onClose, onSubmit, defaultValues, analysenList, zutatendatenbankList, enablePhotoScan = false, enablePhotoLocation = true }: AnalyseergebnisseDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'analyseergebnisse');
      await onSubmit(clean as Analyseergebnisse['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="zugehoerende_analyse" entity="Analysen">\n${JSON.stringify(analysenList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      contextParts.push(`<available-records field="erkannte_zutat" entity="Zutatendatenbank">\n${JSON.stringify(zutatendatenbankList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      const schema = `{\n  "zugehoerende_analyse": string | null, // Display name from Analysen (see <available-records>)\n  "erkannte_zutat": string | null, // Display name from Zutatendatenbank (see <available-records>)\n  "menge_anteil": string | null, // Menge/Anteil\n  "einzelbewertung": LookupValue | null, // Bewertung dieser Zutat (select one key: "allergen_vorhanden" | "positiv" | "neutral" | "negativ") mapping: allergen_vorhanden=Allergen vorhanden, positiv=Positiv, neutral=Neutral, negativ=Negativ\n  "bemerkung_zutat": string | null, // Bemerkung zu dieser Zutat\n}`;
  ...
  const DIALOG_INTENT = defaultValues ? 'Analyseergebnisse bearbeiten' : 'Analyseergebnisse hinzufügen';
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={v => setFields(f => ({ ...f, einzelbewertung: v === 'none' ? undefined : v as any }))}
            >
