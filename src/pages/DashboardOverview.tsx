...
import {
  IconAlertCircle, IconPlus, IconTrash, IconEdit, IconChevronRight, IconChevronDown,
  IconFlask, IconBolt, IconLeaf, IconShieldExclamation, IconCircleCheck, IconCircleX,
  IconAlertTriangle, IconCircle, IconPackage, IconSearch
} from '@tabler/icons-react';

// ─── Rating helpers ────────────────────────────────────────────────

const BEWERTUNG_CONFIG: Record<string, { label: string; color: string; bg: string; icon: React.ReactNode }> = {
  sehr_empfehlenswert: { label: 'Sehr empfehlenswert', color: 'text-emerald-700', bg: 'bg-emerald-50 border-emerald-200', icon: <IconCircleCheck size={14} stroke={1.5} className="text-emerald-600" /> },
  empfehlenswert:      { label: 'Empfehlenswert',      color: 'text-green-700',   bg: 'bg-green-50 border-green-200',   icon: <IconCircleCheck size={14} stroke={1.5} className="text-green-600" /> },
  akzeptabel:          { label: 'Akzeptabel',           color: 'text-amber-700',   bg: 'bg-amber-50 border-amber-200',   icon: <IconAlertTriangle size={14} stroke={1.5} className="text-amber-600" /> },
  nicht_empfehlenswert:{ label: 'Nicht empfehlenswert',color: 'text-orange-700',  bg: 'bg-orange-50 border-orange-200', icon: <IconCircleX size={14} stroke={1.5} className="text-orange-600" /> },
  bedenklich:          { label: 'Bedenklich',           color: 'text-red-700',     bg: 'bg-red-50 border-red-200',       icon: <IconCircleX size={14} stroke={1.5} className="text-red-600" /> },
};
...
export default function DashboardOverview() {
  ...
  const bedenklichCount = useMemo(() =>
    analysen.filter(a => a.fields.gesamtbewertung?.key === 'bedenklich' || a.fields.gesamtbewertung?.key === 'nicht_empfehlenswert').length,
  [analysen]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard
          ...
          description="Gesamt"
          icon={<IconFlask size={18} stroke={1.5} className="text-muted-foreground" />}
        />
        <StatCard
          title="Zutaten"
          value={String(zutatendatenbank.length)}
          description="in Datenbank"
          icon={<IconLeaf size={18} stroke={1.5} className="text-muted-foreground" />}
        />
        <StatCard
          title="Allergene"
          value={String(allergenCount)}
          description="Zutaten markiert"
          icon={<IconShieldExclamation size={18} stroke={1.5} className="text-muted-foreground" />}
        />
        <StatCard
          title="Bedenklich"
          value={String(bedenklichCount)}
          description="Analyseergebnisse"
          icon={<IconAlertCircle size={18} stroke={1.5} className="text-muted-foreground" />}
        />
      </div>
        ...
        <div className="rounded-2xl border bg-card p-4">
          <div className="flex items-center justify-between mb-3">
            <span className="text-sm font-semibold text-foreground flex items-center gap-2">
              <IconBolt size={15} stroke={1.5} className="text-amber-500" />
      ...
      <div className="rounded-2xl border bg-card overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-4 border-b bg-muted/30">
          ...
          <div className="relative max-w-xs w-full">
            <IconSearch size={14} stroke={1.5} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
            <input
              type="text"
        ...
        {filteredAnalysen.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 gap-3 text-center">
            <div className="w-12 h-12 rounded-2xl bg-muted flex items-center justify-center">
              <IconPackage size={20} stroke={1.5} className="text-muted-foreground" />
            </div>
            <div>
            ...
            <Button size="sm" onClick={() => { setEditAnalyse(null); setAnalyseDialogOpen(true); }} className="gap-2 mt-1">
              <IconPlus size={14} stroke={1.5} /> Neue Analyse
            </Button>
          </div>
        ) : (
                  ...
                  >
                    ...
                    <div className="w-10 h-10 rounded-xl bg-muted flex items-center justify-center shrink-0 overflow-hidden">
                      {analyse.fields.produktbild
                        ...
                        : <IconFlask size={16} stroke={1.5} className="text-muted-foreground" />
                      }
                    </div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5 flex items-center gap-2">
                        ...
                        {ergebnisse.length > 0 && (
                          <span className="inline-flex items-center gap-1">
                            <IconCircle size={3} stroke={1.5} className="fill-muted-foreground text-muted-foreground" />
                    ...
                    <div className="bg-muted/20 border-t border-border/50">
                      {ergebnisse.length === 0 ? (
                        <div className="px-14 py-4 text-sm text-muted-foreground flex items-center justify-between">
                          ...
                          >
                            <IconPlus size={12} stroke={1.5} /> Zutat erfassen
                          </Button>
                        </div>
                      ) : (
                        <div className="px-5 py-3 space-y-1">
                          ...
                          <div className="pl-9 pt-1">
                            ...
                            >
                              <IconPlus size={12} stroke={1.5} /> Zutat hinzufügen
                            </Button>
                          </div>
                        </div>
                      )}
